...
ulidFromInteger n
    ...
    | n > maxValidInteger = Left "Value must not be larger than the maximum safe Integer size (128 bits)"
    | otherwise = Right . decode . LBS.pack . (unroll 16) $ n -- 16 bytes = 128 bit
  where
